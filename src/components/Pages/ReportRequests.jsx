...
export function ReportRequests() {
    ...
    useEffect(() => {
        async function fetchReportedUsers() {
            setLoading(true);
            try {
                const reportedUsers = await axios.get(`${serverURL}/api/users/reportedUsers`)
    ...
    async function handleStatusChange(reportId, action) {
        setLoading(true);
        try {
            const response = await axios.post(`${serverURL}/api/users/reportedUser/changeStatus`, {
                reportId,
                action
            });
        ...
        <>
            ...
            <div>
                <Table>
                    ...
                    <tbody>
                        {reportedUsers.map(({ _id, reason, reportedUserId, reporterUserId, status }) => (
                            <tr key={_id} className="border-top">
                                <td>{reportedUserId?.firstName} {reportedUserId?.lastName}</td>
                                <td>{reporterUserId?.firstName} {reporterUserId?.lastName}</td>
                                <td>{status}</td>
                                <td>{reason}</td>
